...
export const getMyOperations = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: tasks, error } = await context.supabase
      .from("tasks")
      .select("id, title, kind, status, progress, detail, priority, started_at, completed_at, heartbeat_at, worker_id, retry_count, max_retries, last_error_code, last_error_message, created_at, updated_at")
      .eq("user_id", uid)
    ...
    const { data: runs } = await context.supabase
      .from("task_runs")
      .select("id, task_id, agent_id, agent_key, attempt, status, started_at, ended_at, heartbeat_at, worker_id, retry_count, failure_code, retryable, duration_ms, inputs, outputs, error, created_at, updated_at, aax_model_id")
      .eq("owner_id", uid)
      .in("task_id", ids)
    ...
    const { data: events } = await context.supabase
      .from("task_events")
      .select("id, task_id, run_id, sequence, event_type, from_status, to_status, message, data, actor_id, worker_id, created_at")
      .in("task_id", ids)
      .order("created_at", { ascending: false })
      .limit(1000);
    return (tasks ?? []).map((task) => ({
      ...task,
      runs: (runs ?? []).filter((run) => run.task_id === task.id),
      events: (events ?? []).filter((event) => event.task_id === task.id).slice(0, 30),
    }));
  });
