...
async function executeTarget(target: EvaluationTarget, input: Record<string, unknown>, db: SupabaseClient) {
  ...
  const tableByTarget: Partial<Record<EvaluationTarget, string>> = { research: "aether_research_sessions", verification: "knowledge_entries", knowledge: "knowledge_entries", reports: "reports", notifications: "notifications", modules: "modules", "battle-versia": "battleversia_games" };
  const table = tableByTarget[target]; if (!table) throw new Error(`No evaluation adapter registered for ${target}`);
  const dbAny = db as SupabaseClient<any>;
  const { error } = await dbAny.from(table).select("id", { head: true, count: "exact" }).limit(1);
