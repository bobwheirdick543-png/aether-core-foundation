...
/** Production-safe single URL retrieval. */
export async function retrievePage(url: string, optionsOrTimeout: RetrieveOptions | number = {}): Promise<RetrievedPage> {
  ...
  if (options.respectRobots && !(await robotsAllowed(normalizedUrl, options.userAgent, options.timeoutMs))) {
    return { url, finalUrl: normalizedUrl, status: 0, title: null, text: "", contentHash: "", retrievedAt, robotsAllowed: false, failureClass: "robots_denied", error: "Retrieval disallowed by robots.txt" };
  }
