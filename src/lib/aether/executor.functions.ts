...
export const advanceResearchRun = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: task } = await supabaseAdmin.from("tasks").select("id, user_id, status, worker_id, lease_expires_at, deadline_at").eq("id", data.taskId).single();
    ...
    if (["queued", "retrying"].includes(run.status)) {
      ...
      const { error: runError } = await supabaseAdmin.from("task_runs").update({ status: "running", worker_id: workerId, lease_expires_at: lease, heartbeat_at: now.toISOString(), started_at: now.toISOString(), updated_at: now.toISOString() }).eq("id", run.id).eq("status", run.status);
      if (runError) throw new Error(runError.message);
      const { error: taskError } = await supabaseAdmin.from("tasks").update({ status: "running", worker_id: workerId, lease_expires_at: lease, heartbeat_at: now.toISOString(), started_at: task.status === "running" ? task.started_at : now.toISOString(), updated_at: now.toISOString() }).eq("id", task.id).in("status", ["queued", "retrying"]);
