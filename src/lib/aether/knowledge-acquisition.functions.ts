...
export const adminPauseKnowledgeAcquisition = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: { taskId: string; reason?: string }) => ({ taskId: String(d.taskId), reason: d.reason?.trim().slice(0, 500) || "Paused by administrator" })).handler(async ({ context, data }) => {
  await requireAdmin(context);
  const { data: task } = await supabaseAdmin.from("tasks").select("id,status,kind,detail,deadline_at").eq("id", data.taskId).maybeSingle();
...
export const adminResumeKnowledgeAcquisition = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: { taskId: string }) => ({ taskId: String(d.taskId) })).handler(async ({ context, data }) => {
  ...
  const detail = task.detail && typeof task.detail === "object" ? task.detail as Record<string, unknown> : {};
  const storedRemaining = Number(detail.remaining_budget_ms);
  const budgetMs = Number.isFinite(storedRemaining) && storedRemaining > 0 ? storedRemaining : null;
  if (detail.pause_requested === true && !budgetMs) throw new Response("The paused acquisition has no remaining research budget; restart it instead", { status: 409 });
  const now = new Date();
  const newDeadline = budgetMs ? new Date(now.getTime() + budgetMs).toISOString() : null;
  const nextDetail = { ...detail, pause_requested: false, resumed_at: now.toISOString() };
  const taskPatch: Record<string, unknown> = { status: "queued", cancel_requested_at: null, cancellation_reason: null, updated_at: now.toISOString(), detail: nextDetail };
  if (newDeadline) taskPatch.deadline_at = newDeadline;
  const { error } = await supabaseAdmin.from("tasks").update(taskPatch).eq("id", task.id).eq("status", "paused");
  if (error) throw new Response(error.message, { status: 500 });
  const { data: job } = await supabaseAdmin.from("aether_knowledge_acquisition_jobs").select("run_id").eq("task_id", task.id).maybeSingle();
  if (job?.run_id) {
    const runPatch: Record<string, unknown> = { status: "queued", cancel_requested_at: null, error: null, retryable: false, next_attempt_at: null, ended_at: null, updated_at: now.toISOString() };
    if (newDeadline) runPatch.deadline_at = newDeadline;
    const { error: runError } = await supabaseAdmin.from("task_runs").update(runPatch).eq("id", job.run_id).eq("status", "paused");
    if (runError) throw new Response(runError.message, { status: 500 });
  }
  ...
  await supabaseAdmin.from("audit_logs").insert({ actor_id: context.userId, action: "knowledge_acquisition.resumed", target_type: "task", target_id: task.id, metadata: { preserved_budget_ms: budgetMs } });
  return { ok: true };
});
