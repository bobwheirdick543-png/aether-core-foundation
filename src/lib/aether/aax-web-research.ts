...
function nativeSourcesToAax(sources: Awaited<ReturnType<typeof runAetherWebResearch>>["sources"]): AaxWebSource[] {
  return sources.map((source, index) => ({ url: source.url, title: source.title, domain: source.domain, snippet: source.snippet, citationIndex: index + 1 }));
}
...
async function executeNative(admin: SupabaseClient, input: { modelKey: string; messages: Array<{ role: "user" | "assistant" | "system"; content: string }>; maxOutputTokens?: number; signal?: AbortSignal; telemetry?: { userId?: string | null; taskId?: string | null; runId?: string | null } }): Promise<AaxWebResponse> {
  ...
  const userQuery = [...input.messages].reverse().find((message) => message.role === "user")?.content?.trim();
  if (!userQuery) throw new Error("Web research requires a user research query");
  const research = await runAetherWebResearch({ query: userQuery, signal: input.signal });
  if (!research.sources.length) throw new Error("Aether web research found no retrievable sources");
  ...
  const response = await executeAaxChat(admin, { modelKey: input.modelKey, messages: [
    ...
    ...input.messages,
    { role: "user", content: `NATIVE AETHER WEB RESEARCH RESULTS (${research.sources.length} retrieved sources across ${research.sourceDomains.length} domains; diversity=${research.diversity.toFixed(2)}):\n${sourceContext}\n\nProduce the best evidence-grounded answer to the user's request.` },
  ], maxOutputTokens: input.maxOutputTokens, signal: input.signal, telemetry: { ...input.telemetry, kind: "aax.chat.native_web_research" } });
  return { content: response.content, sources: nativeSourcesToAax(research.sources), tokensIn: response.tokensIn, tokensOut: response.tokensOut, latencyMs: Date.now() - started, providerModel: response.providerModel };
}
...
async function executeXaiProviderResearch(admin: SupabaseClient, input: { model: Model; messages: Array<{ role: "user" | "assistant" | "system"; content: string }>; maxOutputTokens?: number; signal?: AbortSignal; telemetry?: { userId?: string | null; taskId?: string | null; runId?: string | null } }): Promise<AaxWebResponse> {
  if (input.model.provider !== "xai" || !input.model.provider_model) throw new Error("Provider web research requires an xAI-backed AAX model");
  if (!process.env.XAI_API_KEY) throw new Error("Provider web research is not configured: XAI_API_KEY is missing");
  const started = Date.now();
  ...
  if (!response.ok) throw new Error(`AAX provider web research request failed (${response.status})`);
  ...
  if (!content) throw new Error("Provider web research returned no answer");
  ...
  return { content, sources, tokensIn, tokensOut, latencyMs, providerModel: input.model.provider_model };
}
...
export async function executeAaxWebResearch(admin: SupabaseClient, input: { modelKey: string; messages: Array<{ role: "user" | "assistant" | "system"; content: string }>; maxOutputTokens?: number; signal?: AbortSignal; telemetry?: { userId?: string | null; taskId?: string | null; runId?: string | null } }): Promise<AaxWebResponse> {
  ...
  if (process.env.AETHER_NATIVE_WEB_RESEARCH !== "false") {
    try { return await executeNative(admin, input); }
    catch (error) { if (process.env.AETHER_WEB_RESEARCH_ALLOW_PROVIDER_FALLBACK !== "true") throw error; }
  }
  try { return await executeXaiProviderResearch(admin, { ...input, model }); }
  catch (error) { await recordAaxHealth(admin, model.id, "failure", { error: error instanceof Error ? error.message : String(error), webResearch: true }); throw error; }
}
