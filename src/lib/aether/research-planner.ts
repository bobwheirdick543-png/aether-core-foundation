...
export function createResearchPlan(topic: string, strategy: ResearchPlan["strategy"] = "multi_source"): ResearchPlan {
  ...
  return {
    ...
    sourceRequirements: { minSources: strategy === "source_comparison" ? 4 : 3, minDomains: strategy === "source_comparison" ? 3 : 2, preferredProviders: ["official", "academic", "government", "wikipedia", "reddit", "duckduckgo"] },
...
export async function persistResearchPlan(admin: SupabaseClient, ownerId: string, sessionId: string, plan: ResearchPlan): Promise<string> {
  const { data, error } = await admin.from("aether_research_plans").insert({ owner_id: ownerId, session_id: sessionId, topic: plan.topic, strategy: plan.strategy, queries: plan.queries, source_requirements: plan.sourceRequirements, comparison_rules: plan.comparisonRules, status: "planned" }).select("id").single();
