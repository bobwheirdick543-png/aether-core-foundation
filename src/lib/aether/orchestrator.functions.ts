  ...
  .handler(async ({ context, data }) => {
    ...
    const steps = draft.agents.map((agentKey, index) => ({
      ...
      model_role: draft.capabilities.includes("code")
        ? "aether-code"
        : draft.capabilities.includes("vision")
          ? "aether-vision"
          : draft.capabilities.includes("translation")
            ? "aether-translate"
...
export const getOrchestrationPlan = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: plan, error } = await supabaseAdmin
      ...
      .eq("id", data.planId)
      .maybeSingle();
    if (error || !plan || (plan.owner_id !== context.userId && !(await isAdmin(supabaseAdmin, context.userId))))
...
export const listAgentConversations = createServerFn({ method: "GET" })
  ...
  .inputValidator((data: { agentKey: string }) => ({
    agentKey: assertNonEmpty(data?.agentKey, "Agent key").slice(0, 100),
  }))
  .handler(async ({ context, data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: conversations, error } = await supabaseAdmin
      .from("agent_conversations")
      .select("id, agent_key, title, archived, last_message_at, created_at, updated_at")
...
export const createAgentConversation = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { agentKey: string; title?: string }) => ({
    agentKey: assertNonEmpty(data?.agentKey, "Agent key").slice(0, 100),
    title: String(data?.title ?? "New chat").trim().slice(0, 160) || "New chat",
  }))
  .handler(async ({ context, data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: conversation, error } = await supabaseAdmin
      .from("agent_conversations")
      .insert({ owner_id: context.userId, agent_key: data.agentKey, title: data.title })
      .select("*")
      .single();
    ...
    await supabaseAdmin.from("audit_logs").insert({
      ...
      metadata: { agent_key: data.agentKey },
    });
    return conversation;
  });
...
export const getAgentConversation = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: conversation, error } = await supabaseAdmin
      ...
      .maybeSingle();
    if (error || !conversation || conversation.owner_id !== context.userId)
...
export const appendAgentConversationMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: conversation } = await supabaseAdmin
      .from("agent_conversations")
      .select("id, owner_id, agent_key")
      .eq("id", data.conversationId)
      .maybeSingle();
    if (!conversation || conversation.owner_id !== context.userId)
      throw new Error("Conversation not found or access denied");
    ...
    await supabaseAdmin.from("audit_logs").insert({
      actor_id: context.userId,
      action: `agent_conversation.message.${data.role}`,
      target_type: "agent_conversation",
      target_id: conversation.id,
      metadata: { agent_key: conversation.agent_key },
    });
    return message;
  });
...
export const updateAgentConversationMessage = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: { messageId: string; content: string }) => ({
    messageId: assertNonEmpty(data?.messageId, "Message ID"),
    content: assertNonEmpty(data?.content, "Message").slice(0, 20000),
  }))
  .handler(async ({ context, data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: existing, error: loadError } = await supabaseAdmin
      .from("agent_conversation_messages")
      .select("id, conversation_id, owner_id, metadata, role")
      .eq("id", data.messageId)
      .maybeSingle();
    if (loadError || !existing || existing.owner_id !== context.userId)
      throw new Error("Message not found or access denied");
    if (existing.metadata?.deleted) throw new Error("Cannot edit a deleted message");
    ...
    const { data: updated, error } = await supabaseAdmin
      .from("agent_conversation_messages")
      .update({ content: data.content, metadata: nextMeta })
      .eq("id", data.messageId)
      .eq("owner_id", context.userId)
      .select("*")
      .single();
    if (error || !updated) throw new Error(error?.message || "Failed to update message");
    await supabaseAdmin.from("audit_logs").insert({
      actor_id: context.userId,
      action: "agent_conversation.message.edited",
      target_type: "agent_conversation_message",
      target_id: data.messageId,
      metadata: { conversation_id: existing.conversation_id },
    });
    return updated;
  });
...
export const deleteAgentConversationMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: existing, error: loadError } = await supabaseAdmin
      .from("agent_conversation_messages")
      .select("id, conversation_id, owner_id, metadata")
      .eq("id", data.messageId)
      .maybeSingle();
    if (loadError || !existing || existing.owner_id !== context.userId)
      throw new Error("Message not found or access denied");
    ...
    const { data: updated, error } = await supabaseAdmin
      ...
      .select("*")
      .single();
    if (error || !updated) throw new Error(error?.message || "Failed to delete message");
    await supabaseAdmin.from("audit_logs").insert({
      actor_id: context.userId,
      action: "agent_conversation.message.binned",
      target_type: "agent_conversation_message",
      target_id: data.messageId,
      metadata: { conversation_id: existing.conversation_id },
    });
...
export const recordAgentLearning = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (data: {
      ...
      sourceType: "conversation" | "research" | "verification" | "task" | "knowledge";
      sourceId?: string;
      candidate: Record<string, unknown>;
      confidence?: number;
    }) => ({
      agentKey: assertNonEmpty(data?.agentKey, "Agent key").slice(0, 100),
      sourceAgentKey: data?.sourceAgentKey?.slice(0, 100) || null,
      sourceType: data.sourceType,
      sourceId: data.sourceId || null,
      candidate: data.candidate ?? {},
      confidence:
        data.confidence == null ? null : Math.max(0, Math.min(1, Number(data.confidence))),
    }),
  )
  .handler(async ({ context, data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: row, error } = await supabaseAdmin
      ...
      .insert({
        ...
        source_agent_key: data.sourceAgentKey,
        owner_id: context.userId,
    ...
    await supabaseAdmin.from("audit_logs").insert({
      actor_id: context.userId,
      action: "agent_learning.candidate_created",
      target_type: "agent_learning_record",
      target_id: row.id,
      metadata: { agent_key: data.agentKey, source_agent_key: data.sourceAgentKey },
    });
    return row;
  });

export const approveAgentLearning = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { learningId: string }) => ({
    learningId: assertNonEmpty(data?.learningId, "Learning ID"),
  }))
  .handler(async ({ context, data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    await supabaseAdmin.from("audit_logs").insert({
      actor_id: context.userId,
      action: "agent_learning.approved",
      target_type: "agent_learning_record",
      target_id: row.id,
      metadata: { agent_key: row.agent_key, source_agent_key: row.source_agent_key },
    });
    return row;
  });
...
export const getAgentProductivity = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: runs } = await supabaseAdmin
      .from("task_runs")
      .select("agent_key, status, retry_count, started_at, ended_at")
    ...
    return (agents ?? []).map((agent: any) => {
      ...
      const productivity = total
        ? Math.round((successRate * 0.6 + reliability * 0.25 + cleanExecution * 0.15) * 10000) / 100
        : 0;
      return {
