...
import { appendTaskEvent, createRun, transitionRun, transitionTask } from "./task-service";
...
function dependencySatisfied(step: StepRow, steps: StepRow[]) {
  const deps = Array.isArray(step.dependencies) ? step.dependencies : [];
  return deps.every((dep) => steps.some((candidate) => (candidate.id === dep || candidate.id === String(dep)) && candidate.status === "completed"));
}
...
export async function prepareNextOrchestrationSteps(admin: SupabaseClient, planId: string, actorId: string) {
  ...
  if (planError || !plan) throw new Error("Orchestration plan not found");
  if (plan.owner_id !== actorId) throw new Error("Orchestration access denied");
  ...
  const ready = steps.filter((step) => (step.status === "pending" || step.status === "ready") && dependencySatisfied(step, steps));
  if (ready.length) {
    const ids = ready.map((step) => step.id);
    await admin.from("orchestration_steps").update({ status: "ready" }).in("id", ids).eq("plan_id", planId);
  }
  return { ready, waitingApproval: false };
}

/** Creates a child runtime run for an executable agent step. The universal runtime remains the execution owner. */
export async function createAgentStepRun(admin: SupabaseClient, planId: string, stepId: string, actorId: string) {
  const { data: plan } = await admin.from("orchestration_plans").select("id, task_id, owner_id, run_id").eq("id", planId).maybeSingle();
  ...
  if (!step || !["ready", "pending"].includes(step.status)) throw new Error("Step is not ready for execution");
  if (!step.agent_key) throw new Error("Step has no authorized agent");
  ...
  await traceOrchestrationEvent(admin, { planId, taskId: plan.task_id, runId: run.id, stepId, actorId, actorType: "orchestrator", eventType: "step.delegated", action: "delegate_agent_step", reason: `Delegated step to authorized agent ${step.agent_key}`, decision: "execute", data: { agent_key: step.agent_key, model_role: step.model_role } });
  return run;
}
...
export async function completeAgentStep(admin: SupabaseClient, planId: string, stepId: string, actorId: string, output: Record<string, unknown>) {
  ...
  if (step.run_id) await transitionRun(admin, step.run_id, "completed", { outputs: output });
...
export async function failAgentStep(admin: SupabaseClient, planId: string, stepId: string, actorId: string, reason: string) {
  ...
  const { data: step } = await admin.from("orchestration_steps").select("id, run_id, status").eq("id", stepId).eq("plan_id", planId).maybeSingle();
  if (!step || step.status !== "running") throw new Error("Step is not running");
  if (step.run_id) await transitionRun(admin, step.run_id, "failed", { errorCode: "agent_step_failed", errorMessage: reason });
  await admin.from("orchestration_steps").update({ status: "failed", ended_at: new Date().toISOString(), error: { message: reason } }).eq("id", stepId).eq("plan_id", planId);
  await appendTaskEvent(admin, { taskId: plan.task_id, eventType: "orchestration.step_failed", message: reason.slice(0, 1000), data: { plan_id: planId, step_id: stepId }, actorId });
  await traceOrchestrationEvent(admin, { planId, taskId: plan.task_id, runId: step.run_id, stepId, actorId, actorType: "agent", eventType: "step.failed", action: "fail_agent_step", reason, decision: "escalate", data: {} });
}
