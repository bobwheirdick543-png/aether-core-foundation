...
import { assertPublicDnsTarget } from "./research-network.server";
...
async function makeRuntimeAbortController(admin: SupabaseClient, taskId: string, runId: string, deadline?: string): Promise<{ controller: AbortController; stop: () => void }> {
  ...
  const timer = setInterval(() => { void (async () => { const [{ data: task }, { data: run }] = await Promise.all([admin.from("tasks").select("cancel_requested_at,deadline_at").eq("id", taskId).maybeSingle(), admin.from("task_runs").select("cancel_requested_at,deadline_at").eq("id", runId).maybeSingle()]); const effectiveDeadline = deadline ?? task?.deadline_at ?? run?.deadline_at; if (task?.cancel_requested_at || run?.cancel_requested_at || (effectiveDeadline && new Date(effectiveDeadline).getTime() <= Date.now())) controller.abort(); })().catch(() => undefined); }, 2000);
  return { controller, stop: () => clearInterval(timer) };
}
async function markCancelled(admin: SupabaseClient, taskId: string, runId: string, workerId?: string): Promise<AgentResult> { await transitionRunStatus(admin, runId, "running", "cancelled", { failureCode: "cancelled", retryable: false, workerId }); await transitionTaskStatus(admin, taskId, "running", "cancelled", { workerId }); return { taskId, runId, agentKey: "research", status: "cancelled", errors: ["Research cancelled by runtime"], timestamp: new Date().toISOString() }; }
...
export async function executeResearchStep(admin: SupabaseClient, opts: { taskId: string; runId: string; ownerId: string; projectId?: string | null; urls: string[]; topic?: string; deadlineAt?: string; workerId?: string }): Promise<AgentResult> {
  ...
  try {
    ...
    const safeUrls = opts.urls.filter((u) => isHttpUrl(u)).slice(0, 10); const pages = [];
    for (const url of safeUrls) {
      if (runtimeAbort.controller.signal.aborted) return markCancelled(admin, opts.taskId, opts.runId, opts.workerId);
      await assertPublicDnsTarget(url);
      const remaining = deadline ? Math.max(1000, new Date(deadline).getTime() - Date.now()) : 12000;
      const page = await retrievePage(url, { timeoutMs: Math.min(12000, remaining), maxBytes: 2_000_000, maxRedirects: 5, maxRetries: 2, respectRobots: true, signal: runtimeAbort.controller.signal }); pages.push(page);
    ...
    const researchSessionId = resultForPersistence.sources.length || resultForPersistence.failedSources.length ? await persistAetherWebResearch(admin, { ownerId: opts.ownerId, scope: "agent", query: resultForPersistence.query, result: resultForPersistence, taskId: opts.taskId, runId: opts.runId, projectId }) : null;
