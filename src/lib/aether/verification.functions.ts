...
async function assertOwnedSession(client: SupabaseClient, sessionId: string, ownerId: string) {
  const { data, error } = await client.from("aether_research_sessions").select("id,owner_id,project_id,task_id,run_id,status,query").eq("id", sessionId).eq("owner_id", ownerId).maybeSingle();
...
async function loadSources(client: SupabaseClient, sessionId: string, ownerId: string): Promise<VerificationSource[]> {
  const { data, error } = await client.from("aether_research_sources").select("id,url,title,domain,content,snippet,published_at,updated_at_source,retrieved_at,quality_score,quality_factors,stale_at").eq("session_id", sessionId).eq("owner_id", ownerId).order("retrieved_at", { ascending: false });
  if (error) throw new Response(`Could not load research sources: ${error.message}`, { status: 500 });
  return (data ?? []) as VerificationSource[];
}

export const listMyVerificationRuns = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((data?: { projectId?: string; sessionId?: string }) => ({ projectId: data?.projectId ?? null, sessionId: data?.sessionId ?? null })).handler(async ({ context, data }) => {
  let query = db(context.supabase).from("verification_runs").select("id,project_id,research_session_id,task_id,run_id,status,verifier_version,summary,metrics,error,created_at,updated_at,completed_at").eq("owner_id", context.userId).order("created_at", { ascending: false }).limit(100);
...
export const startVerificationRun = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data: { sessionId: string; claims: string[]; idempotencyKey?: string }) => {
  ...
  const { data: task, error: taskError } = await client.from("tasks").insert({ user_id: context.userId, title: "Verify research claims", kind: "verification", status: "queued", progress: 0, detail: { researchSessionId: data.sessionId, claimCount: data.claims.length } }).select("id").single();
  if (taskError || !task) throw new Response(`Could not create verification task: ${taskError?.message ?? "unknown error"}`, { status: 500 });
  const { data: taskRun, error: taskRunError } = await supabaseAdmin.from("task_runs").insert({ task_id: task.id, owner_id: context.userId, agent_key: "verification", attempt: 1, status: "queued", inputs: { research_session_id: data.sessionId, claims: data.claims }, outputs: {}, idempotency_key: `verification:${task.id}:attempt:1` }).select("id").single();
  ...
  const { data: verificationRun, error } = await supabaseAdmin.from("verification_runs").insert({ owner_id: context.userId, project_id: session.project_id ?? null, research_session_id: session.id, task_id: task.id, run_id: taskRun.id, status: "queued", verifier_version: "g1.0.0", idempotency_key: data.idempotencyKey, metrics: { claimCount: data.claims.length } }).select("id,status,task_id,run_id,project_id,research_session_id,verifier_version").single();
  if (error || !verificationRun) throw new Response(`Could not persist verification run: ${error?.message ?? "unknown error"}`, { status: 500 });
  await client.from("audit_logs").insert({ actor_id: context.userId, action: "verification.queued", target_type: "verification_runs", target_id: verificationRun.id, metadata: { research_session_id: session.id, task_id: task.id, claim_count: data.claims.length } });
  return { ok: true as const, reused: false, ...verificationRun };
});

export const executeVerificationRun = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data: { verificationRunId: string }) => {
  ...
  const client = db(context.supabase);
  const { data: run, error: runError } = await client.from("verification_runs").select("id,owner_id,project_id,research_session_id,task_id,run_id,status,verifier_version").eq("id", data.verificationRunId).eq("owner_id", context.userId).maybeSingle();
  ...
  await supabaseAdmin.from("verification_runs").update({ status: "running", updated_at: startedAt, error: null }).eq("id", run.id).eq("owner_id", context.userId);
  if (run.task_id) {
    await client.from("tasks").update({ status: "running", progress: 10, started_at: startedAt, heartbeat_at: startedAt }).eq("id", run.task_id).eq("user_id", context.userId);
    await supabaseAdmin.from("task_runs").update({ status: "running", started_at: startedAt, heartbeat_at: startedAt }).eq("id", run.run_id).eq("owner_id", context.userId);
  }
  try {
    ...
    for (const result of results) {
      const { data: claimRow, error: claimError } = await supabaseAdmin.from("verification_claims").insert({ verification_run_id: run.id, owner_id: context.userId, claim: result.claim, normalized_claim: result.normalizedClaim, verification_state: result.verificationState, confidence: result.confidence, evidence_strength: result.evidenceStrength, authority_score: result.authorityScore, freshness_score: result.freshnessScore, uncertainty: result.uncertainty, contradiction_count: result.contradictionCount, date_mismatch_count: result.dateMismatchCount, missing_evidence: result.missingEvidence, requires_review: result.requiresReview, review_reason: result.reviewReason, metadata: { sourceCount: sources.length } }).select("id").single();
      ...
      if (result.evidence.length) {
        const evidenceRows = result.evidence.map((evidence) => ({ claim_id: claimRow.id, owner_id: context.userId, source_id: evidence.sourceId, source_url: evidence.sourceUrl, source_title: evidence.sourceTitle, excerpt: evidence.excerpt, supports_claim: evidence.supportsClaim, evidence_strength: evidence.evidenceStrength, authority_score: evidence.authorityScore, freshness_score: evidence.freshnessScore, published_at: evidence.publishedAt, retrieved_at: evidence.retrievedAt, metadata: { reasons: evidence.reasons } }));
    ...
    await supabaseAdmin.from("verification_runs").update({ status: finalStatus, summary, metrics: { claimCount: results.length, sourceCount: sources.length, states: counts, needsReview }, completed_at: completedAt, updated_at: completedAt }).eq("id", run.id).eq("owner_id", context.userId);
    if (run.task_id) {
    ...
    await supabaseAdmin.from("verification_runs").update({ status: "failed", error: message.slice(0, 2000), updated_at: failedAt }).eq("id", run.id).eq("owner_id", context.userId);
    if (run.task_id) {
...
export const getMyVerificationRun = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((data: { verificationRunId: string }) => ({ verificationRunId: String(data?.verificationRunId ?? "").trim() })).handler(async ({ context, data }) => {
  const client = db(context.supabase);
...
export const reviewVerificationClaim = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data: { claimId: string; decision: "accept" | "needs_review" | "reject" | "reverify"; reason?: string }) => ({ claimId: String(data?.claimId ?? "").trim(), decision: data.decision, reason: data.reason?.trim().slice(0, 2000) || null })).handler(async ({ context, data }) => {
  const client = db(context.supabase);
  ...
  const { error } = await supabaseAdmin.from("verification_claims").update({ verification_state: next, requires_review: next !== "verified", review_reason: data.reason }).eq("id", claim.id).eq("owner_id", context.userId);
  if (error) throw new Response(`Could not update verification claim: ${error.message}`, { status: 500 });
  const { error: decisionError } = await supabaseAdmin.from("verification_decisions").insert({ claim_id: claim.id, owner_id: context.userId, actor_id: context.userId, decision: data.decision, previous_state: previous, new_state: next, reason: data.reason });
...
export const cancelVerificationRun = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data: { verificationRunId: string; reason?: string }) => ({ verificationRunId: String(data?.verificationRunId ?? "").trim(), reason: data.reason?.trim().slice(0, 500) || "Cancelled by user" })).handler(async ({ context, data }) => {
  const client = db(context.supabase);
  ...
  await supabaseAdmin.from("verification_runs").update({ status: "cancelled", error: data.reason, updated_at: now }).eq("id", run.id).eq("owner_id", context.userId);
