...
type KnowledgeEntry = { id: string; collection_id: string; title: string; body: string | null; stage: string; current_version: number };
type KnowledgeVersion = { id: string; entry_id: string; version: number; provenance?: unknown };
...
async function productionEntry(client: SupabaseClient, entryId: string, ownerId: string) {
  const { data, error } = await client.from("knowledge_entries").select("id,collection_id,title,body,stage,current_version").eq("id", entryId).eq("owner_id", ownerId).maybeSingle();
  if (error) throw new Response(`Could not load knowledge entry: ${error.message}`, { status: 500 });
  const entry = data as unknown as KnowledgeEntry | null;
  ...
  return { entry, projectId: (collection as { project_id?: string | null } | null)?.project_id ?? null };
}
...
export const runPhaseIIndex = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: { jobId: string }) => ({ jobId: String(d?.jobId ?? "").trim() })).handler(async ({ context, data }) => {
  ...
  try {
    ...
    const chunks = chunkText(entry.body ?? entry.title); const { data: versionData } = await client.from("aether_knowledge_versions").select("id,entry_id,version,provenance").eq("owner_id", context.userId).eq("entry_id", entry.id).eq("version", entry.current_version).maybeSingle(); const version = versionData as unknown as KnowledgeVersion | null;
    ...
    const { data: candidateData } = await client.from("aether_knowledge_candidates").select("id,entities,relations,provenance").eq("owner_id", context.userId).eq("published_entry_id", entry.id).maybeSingle(); const candidate = candidateData as { id: string; entities?: unknown; relations?: unknown; provenance?: Record<string, unknown> } | null;
    ...
    const completed = new Date().toISOString(); await supabaseAdmin.from("aether_retrieval_index_jobs").update({ status: "completed", completed_at: completed }).eq("id", job.id).eq("owner_id", context.userId); return { ok: true as const, reused: false, status: "completed", chunks: chunks.length, version: entry.current_version };
  } catch (error) { const message = error instanceof Error ? error.message : String(error); await supabaseAdmin.from("aether_retrieval_index_jobs").update({ status: "failed", last_error: message.slice(0, 2000) }).eq("id", job.id).eq("owner_id", context.userId); throw new Response("Knowledge indexing failed; the durable job remains retryable.", { status: 500 }); }
});
...
export const retrievePhaseI = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d: { query: string; projectId?: string | null; mode?: RetrievalMode; topK?: number }) => { const query = String(d?.query ?? "").trim().slice(0, 20000), mode = d?.mode ?? "hybrid", topK = Math.max(1, Math.min(20, Number(d?.topK ?? 8))); if (!query) throw new Error("Retrieval query is required"); if (!["lexical", "semantic", "hybrid", "graph"].includes(mode)) throw new Error("Unsupported retrieval mode"); return { query, projectId: d?.projectId ?? null, mode, topK }; }).handler(async ({ context, data }) => {
  ...
  const { data: run, error: re } = await supabaseAdmin.from("aether_retrieval_runs").insert({ owner_id: context.userId, project_id: data.projectId, query: data.query, mode: data.mode, status: "running", top_k: data.topK }).select("id").single(); if (re || !run) throw new Response(`Could not create retrieval run: ${re?.message ?? "unknown error"}`, { status: 500 });
  try {
    ...
    const entryIds = Array.from(new Set((chunks ?? []).map((x) => x.entry_id))); const { data: entryData, error: ee } = entryIds.length ? await client.from("knowledge_entries").select("id,title,stage,current_version").eq("owner_id", context.userId).eq("stage", "production").in("id", entryIds) : { data: [], error: null }; if (ee) throw new Error(ee.message); const entries = (entryData ?? []) as unknown as Array<{ id: string; title: string; stage: string; current_version: number }>;
    const entriesById = new Map<string, { id: string; title: string; stage: string; current_version: number }>(); for (const entry of entries) entriesById.set(entry.id, entry);
    const { data: versionData } = entryIds.length ? await client.from("aether_knowledge_versions").select("id,entry_id,version").eq("owner_id", context.userId).in("entry_id", entryIds) : { data: [] }; const versions = (versionData ?? []) as unknown as KnowledgeVersion[]; const versionMap = new Map<string, string>(); for (const version of versions) versionMap.set(`${version.entry_id}:${version.version}`, version.id);
    const { data: nodeData } = await client.from("aether_knowledge_graph_nodes").select("id,normalized_name,source_entry_id").eq("owner_id", context.userId).limit(1000); const nodes = (nodeData ?? []) as unknown as Array<{ id: string; normalized_name: string; source_entry_id: string | null }>;
    const terms = new Set(data.query.toLowerCase().normalize("NFKC").match(/[\p{L}\p{N}]{2,}/gu) ?? []); const nodeIds = nodes.filter((n) => Array.from(terms).some((t) => n.normalized_name.includes(t))).map((n) => n.id); const { data: edgeData } = nodeIds.length ? await client.from("aether_knowledge_graph_edges").select("source_entry_id,confidence").eq("owner_id", context.userId).in("subject_node_id", nodeIds).limit(1000) : { data: [] }; const edges = (edgeData ?? []) as unknown as Array<{ source_entry_id: string | null; confidence: number }>; const graph = new Map<string, number>(); for (const edge of edges) if (edge.source_entry_id) graph.set(edge.source_entry_id, Math.max(graph.get(edge.source_entry_id) ?? 0, Number(edge.confidence ?? 0.5)));
    const queryEmbedding = data.mode === "lexical" ? null : await deterministicEmbeddingProvider.embed(data.query); const chunkIds = (chunks ?? []).map((x) => x.id); const { data: embeddingData } = chunkIds.length && queryEmbedding ? await client.from("aether_knowledge_embeddings").select("chunk_id,embedding").eq("owner_id", context.userId).eq("is_current", true).in("chunk_id", chunkIds) : { data: [] }; const embeddingRows = (embeddingData ?? []) as unknown as Array<{ chunk_id: string; embedding: unknown }>; const embeddings = new Map<string, unknown>(); for (const row of embeddingRows) embeddings.set(row.chunk_id, row.embedding);
    const ranked = (chunks ?? []).flatMap((chunk) => { const entry = entriesById.get(chunk.entry_id); if (!entry) return []; const lex = lexicalScore(data.query, chunk.content); let sem = 0; const raw = embeddings.get(chunk.id); if (raw && queryEmbedding) { const vals = Array.isArray(raw) ? raw.map(Number) : String(raw).replace(/[\[\]]/g, "").split(",").map(Number); if (vals.length === queryEmbedding.length) sem = Math.max(0, (vals.reduce((s, v, i) => s + v * queryEmbedding[i], 0) + 1) / 2); } const meta = lex.matchedTerms.some((t) => entry.title.toLowerCase().includes(t)) ? 1 : 0; const g = graph.get(entry.id) ?? 0; const score = combineScores({ lexical: lex.score, semantic: sem, metadata: meta, graph: g, mode: data.mode }); return [{ entryId: entry.id, chunkId: chunk.id, version: entry.current_version, title: entry.title, snippet: chunk.content.slice(0, 700), lexicalScore: lex.score, semanticScore: sem, metadataScore: meta, graphScore: g, finalScore: score, matchedTerms: lex.matchedTerms, provenance: { source: "production-knowledge", version: entry.current_version, version_id: versionMap.get(`${entry.id}:${entry.current_version}`) ?? null } }]; }).sort((a, b) => b.finalScore - a.finalScore).slice(0, data.topK);
    const resultRows = ranked.map((r, i) => ({ retrieval_run_id: run.id, owner_id: context.userId, project_id: data.projectId, entry_id: r.entryId, chunk_id: r.chunkId, version_id: versionMap.get(`${r.entryId}:${r.version}`) ?? null, rank: i + 1, lexical_score: r.lexicalScore, semantic_score: r.semanticScore, metadata_score: r.metadataScore, graph_score: r.graphScore, final_score: r.finalScore, matched_terms: r.matchedTerms, snippet: r.snippet, provenance: r.provenance })); if (resultRows.length) { const { error: ins } = await supabaseAdmin.from("aether_retrieval_results").insert(resultRows); if (ins) throw new Error(ins.message); }
