...
export const getPhaseWObservability = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [workers, tasks, runs, events, spans, recentErrors] = await Promise.all([
    ...
    const retryingRuns = allRuns.filter((run) => run.retryable === true || run.status === "retrying").length;
