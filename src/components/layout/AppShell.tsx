...
const NAV = [
  ...
  { label: "Schedules", to: "/schedules" as const, icon: CalendarClock },
  { label: "Battleversia", to: "/battleversia" as const, icon: Swords },
  { label: "Notifications", to: "/notifications" as const, icon: Bell },
  { label: "API Keys", to: "/api-keys" as const, icon: KeyRound },
  { label: "Developer API", to: "/developer-api" as const, icon: Code2 },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-20 items-center gap-3 border-b border-border bg-background/85 px-5 backdrop-blur-xl lg:hidden">
          ...
          <Link
            to="/notifications"
            className="relative ml-auto rounded-md border border-border p-2"
            aria-label={`Notifications${unreadCount ? `, ${unreadCount} unread` : ""}`}
          >
            <Bell className="h-4 w-4" />
            {unreadCount > 0 ? (
              <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-primary px-1 text-center text-[9px] font-semibold text-primary-foreground">
...
export function QuickAction({
  ...
  return (
    <Link to={to} className="panel group flex items-start gap-3 p-4 transition-colors hover:border-primary/40">
