...
import { LayoutDashboard, MessagesSquare, Boxes, FolderKanban, Library, Brain, Telescope, FileText, ListChecks, KeyRound, Settings, Menu, X, LogOut, ShieldCheck, Bell, Activity, Swords } from "lucide-react";
...
const NAV = [
  ...
  { label: "Battleversia", to: "/battleversia" as const, icon: Swords },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { let channel: ReturnType<typeof supabase.channel> | null = null; let cancelled = false; async function connectNotifications(){const {data:{user}}=await supabase.auth.getUser();if(!user||cancelled)return;const {count}=await supabase.from("notifications").select("id",{count:"exact",head:true}).eq("recipient_id",user.id).is("read_at",null);if(!cancelled)setUnreadCount(count??0);channel=supabase.channel(`notifications:${user.id}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"notifications",filter:`recipient_id=eq.${user.id}`},()=>{if(!cancelled)setUnreadCount(c=>c+1);queryClient.invalidateQueries({queryKey:["my-notifications"]});}).subscribe();}void connectNotifications();return()=>{cancelled=true;if(channel)void supabase.removeChannel(channel);};},[queryClient]);
