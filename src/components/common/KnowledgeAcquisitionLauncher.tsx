...
export function KnowledgeAcquisitionLauncher() {
  ...
  return <Panel>
    ...
    <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_150px_140px_auto]"><input value={subject} onChange={(e)=>setSubject(e.target.value)} onKeyDown={(e)=>{if(e.key==="Enter")void submit();}} placeholder="Subject, concept, website URL or public GitHub repository" className="rounded-md border border-border bg-background px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-primary"/><select value={budget} onChange={(e)=>setBudget(e.target.value)} className="rounded-md border border-border bg-background px-3 py-2 text-xs"><option value="5">5 min</option><option value="10">10 min</option><option value="15">15 min (default)</option><option value="20">20 min</option><option value="30">30 min</option><option value="45">45 min</option><option value="60">60 min</option></select><select value={sourceType} onChange={(e)=>setSourceType(e.target.value)} className="rounded-md border border-border bg-background px-3 py-2 text-xs"><option value="background">Background</option><option value="url">Website URL</option><option value="github">Public GitHub</option><option value="prompt">Prompt</option></select><Button size="sm" onClick={()=>void submit()} disabled={busy||!subject.trim()}><Play className="mr-1.5 h-4 w-4"/>Start</Button></div>
