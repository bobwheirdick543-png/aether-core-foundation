...
async function handle(request: Request) {
  const started = Date.now(); const requestId = requestIdFrom(request); let identity: Awaited<ReturnType<typeof authenticateZ2ApiKey>> = null; let statusCode = 500; let idempotencyKey: string | null = null;
  try {
    assertRequestSize(request); identity = await authenticateZ2ApiKey(request); if (!identity) { statusCode = 401; return errorResponse(401, "invalid_api_key", "A valid Aether Ascension API key is required", requestId); }
    ...
    if (idempotencyKey) {
      ...
      const { data: claimRows, error: claimError } = await db.rpc("aether_api_idempotency_claim", { p_api_key_id: identity.apiKeyId, p_idempotency_key: idempotencyKey });
      if (claimError) throw new Error(`Idempotency claim failed: ${claimError.message}`);
      const claim = Array.isArray(claimRows) ? claimRows[0] : claimRows;
      if (claim?.claimed !== true) {
        if (claim?.status === "completed" && claim.response_body) { statusCode = 200; return new Response(JSON.stringify(claim.response_body), { status: 200, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-Request-Id": requestId, "X-AAX-Model": identity.modelKey, "X-Idempotent-Replay": "true", "Access-Control-Allow-Origin": "*" } }); }
        statusCode = 409; return errorResponse(409, "idempotency_in_progress", "An identical request is already being processed or has already failed. Use the original request ID or a new idempotency key.", requestId);
      }
    }
    let body: any; try { body = await request.json(); } catch { statusCode = 400; return errorResponse(400, "invalid_json", "Request body must contain valid JSON", requestId); }
    const result = await executeZ2Intelligence(request, identity, body, requestId); statusCode = result.status;
    if (idempotencyKey) await db.rpc("aether_api_idempotency_complete", { p_api_key_id: identity.apiKeyId, p_idempotency_key: idempotencyKey, p_request_record_id: result.recordId, p_response_body: result.responseBody, p_success: true });
    ...
    if (idempotencyKey && identity) await db.rpc("aether_api_idempotency_complete", { p_api_key_id: identity.apiKeyId, p_idempotency_key: idempotencyKey, p_request_record_id: null, p_response_body: null, p_success: false }).catch(() => undefined);
    ...
    statusCode = 500; return errorResponse(500, "internal_error", "Aether Intelligence API request failed", requestId);
  } finally {
