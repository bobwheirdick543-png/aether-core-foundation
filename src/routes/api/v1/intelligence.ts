...
async function handle(request: Request) {
  ...
  try {
    ...
    if (!identity) {
      statusCode = 401;
      return errorResponse(401, "invalid_api_key", "A valid Aether Ascension API key is required", requestId);
    }
    if (identity.status !== "active") {
      statusCode = 403;
      return errorResponse(403, "api_key_not_active", "This Aether API key is not currently authorized", requestId);
    }
    if (request.headers.get("content-type")?.includes("application/json") !== true) {
      statusCode = 415;
      return errorResponse(415, "unsupported_media_type", "Requests must use application/json", requestId);
    }
    ...
    if (idempotencyKey) {
      if (idempotencyKey.length > 255) {
        statusCode = 400;
        return errorResponse(400, "invalid_idempotency_key", "Idempotency-Key must be 255 characters or fewer", requestId);
      }

      const { data: claimRows, error: claimError } = await db.rpc("aether_api_idempotency_claim", {
        p_api_key_id: identity.apiKeyId,
        p_owner_id: identity.ownerId,
        p_idempotency_key: idempotencyKey,
        p_request_hash: requestFingerprint(body),
      });

      if (claimError) {
        if (claimError.message?.includes("idempotency_conflict")) {
          statusCode = 409;
          return errorResponse(409, "idempotency_conflict", "This Idempotency-Key was already used with a different request body", requestId);
        }
      ...
      if (claim?.claimed !== true) {
        if (claim?.status_code && claim.response_body) {
          ...
          return new Response(JSON.stringify(claim.response_body), {
            status: statusCode,
            headers: {
              "Content-Type": "application/json; charset=utf-8",
              "Cache-Control": "no-store",
              "X-Request-Id": requestId,
              "X-AAX-Model": identity.modelKey,
    ...
    if (idempotencyKey && idempotencyClaimed) {
      await db.rpc("aether_api_idempotency_complete", {
        p_api_key_id: identity.apiKeyId,
        p_idempotency_key: idempotencyKey,
        p_request_record_id: result.recordId,
        p_response_body: result.responseBody,
        p_success: result.status >= 200 && result.status < 300,
      });
    }
    ...
    if (idempotencyKey && identity && idempotencyClaimed) {
      await db
        .rpc("aether_api_idempotency_complete", {
          p_api_key_id: identity.apiKeyId,
          p_idempotency_key: idempotencyKey,
          p_request_record_id: null,
    ...
    if (error instanceof Response) {
      ...
      error.headers.set("Access-Control-Allow-Origin", "*");
      return error;
    }
    statusCode = 500;
    return errorResponse(500, "internal_error", "Aether Intelligence API request failed", requestId);
  } finally {
    await db.from("aether_api_logs").insert({
      api_key_id: identity?.apiKeyId ?? null,
      ...
      api_version: "z2",
      application_name: identity?.applicationName ?? null,
      environment: identity?.environment ?? null,
      model_id: identity?.modelId ?? null,
      model_key: identity?.modelKey ?? null,
      metadata: {
        apiKind: "aax",
        modelGeneration: identity?.modelGeneration ?? null,
