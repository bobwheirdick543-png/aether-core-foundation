...
import { MessagesSquare, Boxes, Bot, Brain, Library, Globe2, FolderKanban, KeyRound, ArrowRight, Sparkles, BellRing } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Aether is an evolving AI platform for conversations, Aether Ascension models, agents, research, memory, knowledge and applications." },
    { property: "og:title", content: "Aether — Your AI Platform" },
    { property: "og:description", content: "One platform. Evolving intelligence. Aether Ascension models, agents, knowledge and research." },
  ] }),
  component: Landing,
});

const FEATURES = [
  { title: "AI Chat", description: "Persistent conversations with a selected Aether Ascension generation and model-specific context.", icon: MessagesSquare },
  { title: "Aether Ascension", description: "A unified evolving AAX model family with generations, capabilities and specializations.", icon: Boxes },
  { title: "AI Agents", description: "Scoped internal agents that research, verify, acquire, curate, report, notify and operate under hard permissions.", icon: Bot },
  ...
  { title: "Developer API", description: "Scoped, revocable API keys for external applications and future Aether integrations.", icon: KeyRound },
];
...
function Landing() {
  return <div className="min-h-screen bg-background">
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-5 py-28 lg:px-8 lg:py-40">
        <div className="animate-in-up max-w-4xl">
          ...
          <p className="mt-6 max-w-3xl text-base leading-relaxed text-muted-foreground sm:text-lg">Aether brings conversations, evolving Aether Ascension intelligence, autonomous background agents, research, memory, knowledge and applications into one platform.</p>
    ...
    <Section eyebrow="Agents" title="A coordinated internal AI workforce" description="Aether's agents have explicit responsibilities, tools and permissions. The Orchestrator coordinates them; specialist agents do the specialist work."><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{AGENTS.map((a) => <div key={a.key} className="panel group p-5 transition-all duration-200 hover:border-primary/35"><div className="flex items-center gap-2"><AetherMark className="h-4 w-4 text-primary opacity-70" /><h3 className="text-sm font-semibold">{a.name}</h3></div><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{a.description}</p></div>)}</div></Section>

    <Section eyebrow="Developers" title="The Aether API" description="External applications can connect through scoped, revocable API keys. AAX is the model family exposed by the platform rather than a collection of unrelated specialist model brands."><div className="panel overflow-x-auto p-6 font-mono text-xs leading-relaxed text-muted-foreground"><pre>{`POST /v1/chat\nAuthorization: Bearer aeth_live_••••••••\nContent-Type: application/json\n\n{\n  "model": "aax-3.1",\n  "messages": [{ "role": "user", "content": "..." }]\n}`}</pre></div><div className="mt-6 flex flex-wrap gap-3"><Button asChild variant="outline"><Link to="/developers">Developer overview</Link></Button><Button asChild variant="outline"><Link {...loginTo("/api-keys")}>API keys workspace</Link></Button></div></Section>
