-- AETHER: strengthen one-click activation while allowing agents without an
-- existing contract row to be initialized from their governed registry record.
...
BEGIN
  ...
  IF NOT FOUND THEN
    SELECT COALESCE(
      jsonb_agg(
        jsonb_build_object(
          ...
          'allowed', ap.allowed,
          'requiresApproval', ap.requires_approval
        )
    ...
    v_definition := jsonb_build_object(
      ...
      'timeout_policy', jsonb_build_object(
        'timeoutMs', 300000,
        'maxRetries', 3,
        'backoffMs', 5000,
      ...
      'telemetry_configuration', jsonb_build_object(
        'collectMetrics', true,
    ...
    IF jsonb_typeof(v_agent.config) = 'object' AND v_agent.config <> '{}'::jsonb THEN
      v_definition := v_definition || v_agent.config;
      v_definition := jsonb_set(v_definition, '{agent_id}', to_jsonb(('agent_' || v_agent.agent_key)::text), true);
      v_definition := jsonb_set(v_definition, '{version}', to_jsonb('1.0.0'::text), true);
    END IF;
  ...
  IF EXISTS (
    SELECT 1
    FROM jsonb_array_elements(v_definition->'permissions') permission
  ...
  IF v_definition->>'agent_id' <> 'agent_' || v_agent.agent_key THEN
    RAISE EXCEPTION 'Agent contract preflight failed: agent identity mismatch';
  END IF;
