-- Fix PL/pgSQL output-column ambiguity in the Phase A worker claim function.
...
BEGIN
 ...
 SELECT tr.* INTO v_run FROM public.task_runs tr JOIN public.tasks t ON t.id=tr.task_id WHERE tr.status IN ('queued','retrying') AND t.status IN ('queued','retrying') AND tr.cancel_requested_at IS NULL AND t.cancel_requested_at IS NULL AND (tr.next_attempt_at IS NULL OR tr.next_attempt_at<=v_now) AND (t.next_attempt_at IS NULL OR t.next_attempt_at<=v_now) AND (tr.deadline_at IS NULL OR tr.deadline_at>v_now) AND (t.deadline_at IS NULL OR t.deadline_at>v_now) ORDER BY t.priority DESC,COALESCE(tr.next_attempt_at,t.next_attempt_at,t.created_at),tr.created_at FOR UPDATE OF tr SKIP LOCKED LIMIT 1;
