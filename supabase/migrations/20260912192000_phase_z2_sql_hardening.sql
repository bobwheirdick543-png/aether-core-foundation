...
as $$
declare k public.aether_api_keys%rowtype; v_period date:=date_trunc('month',now())::date; u public.aether_api_key_usage_periods%rowtype; reservation uuid; requested integer:=greatest(1,p_requested_tokens);
begin
...
on conflict on constraint aether_api_key_usage_periods_pkey do update set request_count=public.aether_api_key_usage_periods.request_count+1,updated_at=now();
else
...
on conflict on constraint aether_api_key_usage_periods_pkey do update set tokens_reserved=public.aether_api_key_usage_periods.tokens_reserved+excluded.tokens_reserved,request_count=public.aether_api_key_usage_periods.request_count+1,updated_at=now()
...
language plpgsql security definer set search_path=public
as $$
declare r public.aether_api_token_reservations%rowtype; actual integer:=greatest(0,p_actual_tokens); released integer; final_status text;
begin
select * into r from public.aether_api_token_reservations where id=p_reservation_id for update;if not found then return;end if;if r.status<>'reserved' then return query select r.consumed_tokens,0,r.period_start;return;end if;
actual:=least(actual,r.reserved_tokens);released:=greatest(0,r.reserved_tokens-actual);final_status:=case when actual>0 then 'consumed' else 'released' end;
...
as $$ select u.period_start,u.token_limit,u.tokens_reserved,u.tokens_consumed,u.request_count,greatest(0,u.token_limit-u.tokens_reserved-u.tokens_consumed) from public.aether_api_key_usage_periods u where u.api_key_id=p_api_key_id and u.period_start=date_trunc('month',now())::date; $$;
