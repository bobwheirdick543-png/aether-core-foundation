...
-- This migration mirrors the verified live-project Z2 fixes applied on 2026-09-12:
-- atomic rate windows with stale cleanup, exact quota finalization (including unlimited keys),
-- body-hash idempotency/security-definer grants, and AAX authentication lifecycle handling.
...
begin
  ...
  on conflict(api_key_id,window_start) do update set request_count=public.aether_api_rate_windows.request_count+1
...
begin
  ...
  if coalesce(key_unlimited,false) then
    ...
    return query select actual,0,r.period_start;
    return;
  end if;
