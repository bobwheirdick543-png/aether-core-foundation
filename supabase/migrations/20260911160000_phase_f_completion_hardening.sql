-- Phase F completion hardening. Additive only.
ALTER TABLE public.aether_research_sources ADD COLUMN IF NOT EXISTS stale_reason text, ADD COLUMN IF NOT EXISTS redirect_chain jsonb NOT NULL DEFAULT '[]'::jsonb, ADD COLUMN IF NOT EXISTS change_state text;
...
ALTER TABLE public.aether_research_policy_events ADD COLUMN IF NOT EXISTS project_id uuid REFERENCES public.projects(id) ON DELETE SET NULL, ADD COLUMN IF NOT EXISTS task_id uuid REFERENCES public.tasks(id) ON DELETE SET NULL, ADD COLUMN IF NOT EXISTS run_id uuid REFERENCES public.task_runs(id) ON DELETE SET NULL;
