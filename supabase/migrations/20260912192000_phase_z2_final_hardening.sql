-- Phase Z2 final hardening: atomic rate windows, unlimited-token accounting, and explicit credential scope persistence.
...
begin
  ...
  if coalesce(key_unlimited,false) then
    -- Unlimited keys reserve zero in advance, but actual provider usage is still tracked.
    final_status := case when p_success and actual > 0 then 'consumed' else 'released' end;
    update public.aether_api_token_reservations
      set consumed_tokens=actual,status=final_status,finalized_at=now()
      where id=r.id;
    update public.aether_api_key_usage_periods
      set tokens_consumed=tokens_consumed+actual,updated_at=now()
  ...
  update public.aether_api_key_usage_periods
    set tokens_reserved=greatest(0,tokens_reserved-r.reserved_tokens),
        tokens_consumed=tokens_consumed+case when p_success then actual else 0 end,
        updated_at=now()
    where api_key_id=r.api_key_id and period_start=r.period_start;
...
-- Keep the legacy Developer API authentication contract intact; Z2 uses its own function.
-- No legacy function signature is altered here.
