...
create table if not exists public.evaluation_runs (
  id uuid primary key default gen_random_uuid(),
  test_case_id uuid not null references public.evaluation_test_cases(id) on delete restrict,
  owner_id uuid not null references auth.users(id) on delete cascade,
  ...
  score numeric(8,5) not null default 0 check (score >= 0 and score <= 1),
  passed boolean not null default false,
  latency_ms integer check (latency_ms is null or latency_ms >= 0),
  failure_rate numeric(8,5) not null default 0,
  retry_rate numeric(8,5) not null default 0,
  approval_rate numeric(8,5) not null default 0,
...
revoke all on public.evaluation_test_cases from anon, authenticated;
revoke all on public.evaluation_runs from anon, authenticated;
revoke all on public.evaluation_run_events from anon, authenticated;
grant select, insert, update, delete on public.evaluation_test_cases to authenticated;
grant select on public.evaluation_runs to authenticated;
grant select on public.evaluation_run_events to authenticated;

drop trigger if exists evaluation_test_cases_updated_at on public.evaluation_test_cases;
create trigger evaluation_test_cases_updated_at before update on public.evaluation_test_cases for each row execute function public.set_updated_at();
