...
select '20260912173000',array['approval notification trigger now uses generic conflict handling compatible with the existing partial idempotency index'],'knowledge_acquisition_notification_conflict_fix','bobwheirdick543@gmail.com',null,array[]::text[]
where not exists(select 1 from supabase_migrations.schema_migrations where version='20260912173000');
