...
create table if not exists public.aax_chat_attachments (
  id uuid primary key default gen_random_uuid(), conversation_id uuid not null references public.aax_conversations(id) on delete cascade,
  message_id uuid references public.aax_conversation_messages(id) on delete set null, owner_id uuid not null references auth.users(id) on delete cascade,
  storage_path text not null unique, filename text not null, mime_type text not null,
  size_bytes bigint not null check (size_bytes > 0 and size_bytes <= 26214400), sha256 text,
  ...
  extracted_text text, metadata jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.aax_chat_sources (
  id uuid primary key default gen_random_uuid(), conversation_id uuid not null references public.aax_conversations(id) on delete cascade,
  message_id uuid references public.aax_conversation_messages(id) on delete cascade, owner_id uuid not null references auth.users(id) on delete cascade,
  url text not null, title text, domain text, snippet text, citation_index integer, created_at timestamptz not null default now()
);
...
create table if not exists public.aax_chat_memory_candidates (
  id uuid primary key default gen_random_uuid(), conversation_id uuid not null references public.aax_conversations(id) on delete cascade,
  message_id uuid not null references public.aax_conversation_messages(id) on delete cascade, owner_id uuid not null references auth.users(id) on delete cascade,
  content text not null, status text not null default 'candidate' check (status in ('candidate','approved','rejected','promoted')), created_at timestamptz not null default now(), reviewed_at timestamptz
);
...
create table if not exists public.aax_chat_generation_runs (
  id uuid primary key default gen_random_uuid(), conversation_id uuid not null references public.aax_conversations(id) on delete cascade,
  owner_id uuid not null references auth.users(id) on delete cascade, model_id uuid references public.aax_models(id) on delete set null,
  user_message_id uuid references public.aax_conversation_messages(id) on delete set null, assistant_message_id uuid references public.aax_conversation_messages(id) on delete set null,
  status text not null default 'running' check (status in ('running','completed','failed','cancelled')), web_research boolean not null default false,
  started_at timestamptz not null default now(), completed_at timestamptz, error text, metadata jsonb not null default '{}'::jsonb
);
