...
begin
  ...
  if k.status <> 'active' or k.revoked_at is not null or (k.expires_at is not null and k.expires_at <= now()) then
    return query select false,'api_key_not_active',null::uuid,period,k.monthly_token_limit,0::bigint,0::bigint;
    return;
  end if;
  ...
  if not k.unlimited_tokens and requested > k.monthly_token_limit then
    return query select false,'monthly_token_quota_exhausted',null::uuid,period,k.monthly_token_limit,0::bigint,0::bigint;
    return;
  end if;
...
begin
  ...
  update public.aether_api_key_usage_periods set tokens_reserved=greatest(0,tokens_reserved-r.reserved_tokens),tokens_consumed=tokens_consumed+actual,updated_at=now() where api_key_id=r.api_key_id and period_start=r.period_start;
