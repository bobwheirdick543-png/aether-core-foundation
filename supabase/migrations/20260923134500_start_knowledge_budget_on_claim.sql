-- Start knowledge-acquisition budgets when a worker actually claims the mission.
-- Queued time is not charged against the selected research budget.
...
BEGIN
 ...
 SELECT tr.* INTO v_run FROM public.task_runs tr JOIN public.tasks t ON t.id=tr.task_id WHERE tr.status IN ('queued','retrying') AND t.status IN ('queued','retrying','running') AND tr.cancel_requested_at IS NULL AND t.cancel_requested_at IS NULL AND (tr.next_attempt_at IS NULL OR tr.next_attempt_at<=v_now) AND (t.next_attempt_at IS NULL OR t.next_attempt_at<=v_now) AND (tr.deadline_at IS NULL OR tr.deadline_at>v_now) AND (t.deadline_at IS NULL OR t.deadline_at>v_now) AND NOT EXISTS (SELECT 1 FROM public.task_runs active WHERE active.task_id=t.id AND active.status='running' AND active.lease_expires_at>v_now) ORDER BY t.priority DESC,COALESCE(tr.next_attempt_at,t.next_attempt_at,t.created_at),tr.created_at FOR UPDATE OF tr SKIP LOCKED LIMIT 1;
 ...
 PERFORM public.append_task_event(v_task.id,v_run.id,'run.claimed','queued','running','Runtime worker claimed execution',jsonb_build_object('attempt',v_run.attempt,'deadline_at',v_task.deadline_at),NULL,p_worker_id);
