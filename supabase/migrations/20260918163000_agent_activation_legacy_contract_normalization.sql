-- AETHER: normalize legacy/partial agent contracts during one-click activation.
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    VALUES(
      p_agent_id, '1.0.0', 'draft', v_definition, md5(v_definition::text), p_actor_id
    )
    ...
    IF v_version.id IS NULL THEN
      SELECT * INTO v_version
      FROM public.agent_versions
      WHERE agent_id = p_agent_id AND version = '1.0.0'
      FOR UPDATE;
    END IF;
  END IF;
  ...
  -- Backfill only missing contract fields. Existing governed values win.
  v_definition := v_defaults || v_definition;
  ...
  SET lifecycle_state = 'active',
      ...
      config_hash = md5(v_definition::text),
