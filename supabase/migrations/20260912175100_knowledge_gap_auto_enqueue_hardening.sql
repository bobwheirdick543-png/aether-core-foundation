...
begin
  ...
  if normalized !~ '(what is|who is|how does|how do|why does|why is|explain|tell me about|research|look up|learn about|everything about|latest|current|compare|difference between|meaning of|define|documentation|docs|how to)' then return new; end if;
  if coalesce(new.metadata->>'backgroundKnowledgeGap','') = 'true' then return new; end if;
  select count(*) into recent_owner_count from public.aether_knowledge_acquisition_jobs where owner_id = new.owner_id and created_at > now_ts - interval '15 minutes' and status in ('queued','running','paused','waiting_approval');
  ...
  select count(*) into existing_count from public.aether_knowledge_acquisition_jobs where dedupe_key = dedupe and status in ('queued','running','paused','waiting_approval');
  if existing_count > 0 then return new; end if;
  scope := jsonb_build_object('core', jsonb_build_array(subject || ' identity and defining characteristics', subject || ' major facts and developments'),'essentialContext', jsonb_build_array(subject || ' essential context required to answer the request'),'relevantRelationships', jsonb_build_array(subject || ' directly relevant relationships or comparisons'),'terminology', jsonb_build_array('terminology needed to understand ' || subject),'verification', jsonb_build_array('cross-source agreement, dates, source quality and unresolved claims'),'exclusions', jsonb_build_array('unrelated domains','recursive expansion not necessary to answer the request'));
  insert into public.tasks(user_id, project_id, title, kind, status, priority, timeout_ms, detail, idempotency_key)
  values(new.owner_id, null, 'Knowledge gap: ' || left(subject, 120), 'knowledge-acquisition', 'queued', 10, 900000, jsonb_build_object('subject', subject, 'scope', scope, 'depth_tier','A','time_budget_ms',900000,'target_type','global','target_model_keys','{}','source_type','background','trigger','background_knowledge_gap','origin_message_id',new.id), dedupe)
