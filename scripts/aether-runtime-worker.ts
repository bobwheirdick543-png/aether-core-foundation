...
async function executeClaim(claim: any): Promise<void> {
  ...
  try {
    ...
    if (pausedByAdmin && currentRun?.status === "running") {
      ...
      await safeTelemetry({ context, level: "info", component: "runtime-worker", eventType: "runtime.run_paused", message: "Runtime stopped at administrator pause boundary", metadata: { taskKind } });
      await span.finish("completed");
      return;
    }
